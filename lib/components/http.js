...
var middleware = {};
middleware.logger = require('morgan');
middleware.json = require('body-parser').json;
middleware.raw = require('body-parser').raw;
middleware.text = require('body-parser').text;
middleware.urlencoded = require('body-parser').urlencoded;
middleware.methodOverride = require('method-override');
middleware.errorHandler = require('errorhandler');
...
var Http = function(app, opts) {
  ...
  this.host = opts.clientHost || DEFAULT_HOST;
  ...
  this.http.use(middleware.json());
  this.http.use(middleware.raw());
  this.http.use(middleware.urlencoded({extended: true}));
  this.http.use(middleware.text({type : '*/*'}));
  this.http.use(middleware.methodOverride());
  
  if(!!opts.useSession){
    middleware.session = require('express-session');
    
    var sessConfig = {};
    sessConfig['resave'] = false;
    sessConfig['saveUninitialized'] = false;
    sessConfig['cookie'] = {};
    if(!!opts.sessSecret){
      sessConfig['secret'] = opts.sessSecret;
    }
    if(!!opts.sessMaxAge){
      sessConfig['cookie']['maxAge'] = opts.sessMaxAge;
    }
    
    if(!!opts.storeType){
      if(opts.storeType == 'redis'){
        var storeConfig = {host: opts.storeHost, port: opts.storePort};
        middleware.RedisStore = require('connect-redis')(middleware.session);
        sessConfig.store = new middleware.RedisStore(storeConfig);
      }
    }
    
    this.http.use(middleware.session(sessConfig));
  }
  ...
  if(!!opts.useCors){
    ...
    this.http.get( "/crossdomain.xml", onCrossDomainHandler );
  }

  var self = this;
  this.app.configure(function() {
    self.http.use(middleware.errorHandler());
  });
...
Http.prototype.loadRoutes = function() {
  ...
  */
  ...
  this.http.all('*', function(req, res) {
